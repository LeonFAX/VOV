...
import { ArrowLeft, Search, Plus, Edit2, Trash2, Calendar, AlertTriangle } from 'lucide-react';
...
export function EventsListPage() {
  ...
  const { events, deleteEvent } = useContentStore();
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-[#2D2619] font-serif flex items-center gap-3">
                <Calendar className="w-8 h-8 text-[#166534]" />
                {t('admin.manageEventsTitle')}
              </h1>
              <p className="text-[#8A7D6E] mt-1">{events.length} {t('admin.records')}</p>
            </div>
            <Button className="bg-[#166534] hover:bg-[#166534]/90" onClick={() => navigate('/admin/events/new')}>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#3D3225]">
                  ...
                  <th className="text-left px-6 py-4 text-[#8A7D6E] text-sm font-medium">{t('admin.tableType')}</th>
                ...
                {filteredEvents.map((event, index) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <span className="text-[#2D2619] font-medium">{event.title}</span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs px-2 py-1 rounded ${
                        event.type === 'battle' ? 'bg-[#8B3A3A]/20 text-[#DC2626]' :
                        event.type === 'operation' ? 'bg-[#166534]/20 text-[#22C55E]' :
                        event.type === 'political' ? 'bg-[#8B6914]/20 text-[#8B6914]' :
                        'bg-[#8A7D6E]/20 text-[#8A7D6E]'
                      }`}>
                        {event.type === 'battle' ? t('admin.typeBattle') :
                         event.type === 'operation' ? t('admin.typeOperation') :
                         event.type === 'political' ? t('admin.typePolitical') : t('admin.typeOther')}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-[#8A7D6E]">{event.location || '—'}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Button variant="ghost" size="icon" className="text-[#8A7D6E] hover:text-[#8B6914]" onClick={() => navigate(`/admin/events/${event.id}/edit`)}>
                          <Edit2 className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="icon" className="text-[#8A7D6E] hover:text-[#DC2626]" onClick={() => setDeleteId(event.id)}>

...
import { ArrowLeft, Search, Plus, Edit2, Trash2, Mail, AlertTriangle } from 'lucide-react';
...
export function LettersListPage() {
  ...
  const { letters, deleteLetter } = useContentStore();
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-[#2D2619] font-serif flex items-center gap-3">
                <Mail className="w-8 h-8 text-[#8B6914]" />
                {t('admin.manageLettersTitle')}
              </h1>
              <p className="text-[#8A7D6E] mt-1">{letters.length} {t('admin.records')}</p>
            </div>
            <Button className="bg-[#8B6914] text-[#2D2619] hover:bg-[#8B6914]/90" onClick={() => navigate('/admin/letters/new')}>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#3D3225]">
                  <th className="text-left px-6 py-4 text-[#8A7D6E] text-sm font-medium">{t('admin.tableDate')}</th>
                  <th className="text-left px-6 py-4 text-[#8A7D6E] text-sm font-medium">{t('admin.letterAuthor')}</th>
                  <th className="text-left px-6 py-4 text-[#8A7D6E] text-sm font-medium">{t('admin.letterRecipient')}</th>
                  <th className="text-left px-6 py-4 text-[#8A7D6E] text-sm font-medium">{t('admin.letterPreview')}</th>
                ...
                {filteredLetters.map((letter, index) => (
                  ...
                  >
                    <td className="px-6 py-4 text-[#8A7D6E]">
                      {format(letter.date, 'dd.MM.yyyy')}
                    </td>
                    <td className="px-6 py-4 text-[#2D2619] font-medium">{letter.author}</td>
                    <td className="px-6 py-4 text-[#8A7D6E]">{letter.recipient}</td>
                    <td className="px-6 py-4 text-[#B8A080] text-sm max-w-[300px] truncate">
                      {letter.text.slice(0, 60)}...
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Button variant="ghost" size="icon" className="text-[#8A7D6E] hover:text-[#8B6914]" onClick={() => navigate(`/admin/letters/${letter.id}/edit`)}>
                          <Edit2 className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="icon" className="text-[#8A7D6E] hover:text-[#DC2626]" onClick={() => setDeleteId(letter.id)}>
